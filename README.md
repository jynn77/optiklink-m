...
- **Turnstile 令牌偶发拿不到。** Cloudflare 的风险评分每次不同，偶尔一次不放行。脚本会自动重试：令牌没放行就再点一次 `uc_gui_click_captcha()` 并重等，最多 5 轮（`TURNSTILE_MAX_RETRY`）后才报失败。如果连重试都用完仍失败，才会整次运行失败，等 cron 下次补。
